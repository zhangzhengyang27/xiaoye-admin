<template>
  <div ref="historyRef" class="history">
    ...
    <template v-if="collectList.length">
      <div :style="titleStyle">收藏 {{ collectList.length > 1 ? '可拖拽排序' : '' }}</div>
...
import { useResizeObserver, delay } from '@pureadmin/utils'
...
function handleScroll(index: number) {
  ...
  const curRef = Array.isArray(curInstance) ? (curInstance[0] as ElRef) : (curInstance as ElRef)
